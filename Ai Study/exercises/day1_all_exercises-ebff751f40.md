...
1. **`.head()`方法**（/hɛd/ 查看前几行）
   ...
   - 为什么查看数据时常用这个方法而不是直接打印全部数据？
    ==因为有的文件内容会很多==
1. **DataFrame概念**（/ˈdeɪtə freɪm/ 数据框）
...
```python
# 读取评分数据文件
ratings_df = pd.read_csv('data/ratings.csv')  # 填入函数名和文件名
...
**任务描述**：
在Linux环境中，你需要为电影数据文件创建一个备份，并验证备份是否成功。

**具体步骤**：
1. 在`data/`目录下创建一个名为`backup/`的子目录
2. 将`data/movies_sample.csv`文件复制到`backup/`目录，并重命名为`movies_backup.csv`
3. 查看`backup/`目录的内容，确认文件已存在
4. 查看备份文件的前3行内容，确认数据正确复制

**请写出完整的命令序列**：
```bash
# 步骤1：创建backup目录（如果不存在）
________ data/backup

# 步骤2：复制并重命名文件
________ data/movies_sample.csv data/backup/movies_backup.csv

# 步骤3：查看backup目录内容
________ data/backup/

# 步骤4：查看备份文件前3行
________ data/backup/movies_backup.csv | ________ -3
```
...
# 步骤2：复制并重命名文件
cp data/movies_sample.csv data/backup/movies_backup.csv

# 步骤3：查看backup目录内容
ls data/backup/

# 步骤4：查看备份文件前3行
cat data/backup/movies_backup.csv | head -3
```
