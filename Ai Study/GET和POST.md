...
`video`
- 视频、影像
- 告诉服务器，这是一种视频数据，而不是音频或图片。
`mp4`
- 告诉服务器，这个视频文件的具体格式是 MP4。
- MP4 是最常见的视频容器格式，可以包含视频、音频、字幕。
### **上传ZIP压缩包** `application/zip`
`application`
- 应用程序数据格式
- 告诉服务器，这不是纯文本，而是一种应用程序数据格式。
`zip`
- 告诉服务器，这个文件的具体格式是 ZIP。
- ZIP 可以把多个文件打包在一起，并进行压缩。
### **上传CSV文件** `text/csv`
`text`
- 文本
- 告诉服务器这是一种文本数据
`csv`
- 告诉服务器，这个文本文件的具体格式是 CSV。
- CSV 文件用逗号（或其他分隔符）来分隔数据，常用于表格和数据交换。
举例说明：
```code
name,age
md,20
